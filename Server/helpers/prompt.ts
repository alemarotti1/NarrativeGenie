export const historiaPrompt = (prompt: string) => {
  let result = `preencha o json com os dados de um mundo fictício descrito por "${prompt.trim()}". Adicione coisas como: as espécies/raças mais comuns, moedas, tecnologias, tipo de governo, religião, etc na descrição. Além disso, digite ao menos dois paragrafos na descrição.`;
...
export const outroPrompt = (prompt: string) => {
  let result = `preencha o json com os dados de um objeto descrito por "${prompt.trim()}". onde "descricao" é uma lista de adjetivos do objeto e "descricao_fisica_em_ingles" tem a lista em inglês das características físicas mais importantes. Adicione coisas como formato, cor, material em "descricao_fisica_em_ingles" e digite ao menos dois paragrafos em "descricao", descrevendo uso, história, etc. `;
  result += `{ nome: string; descricao: string; descricao_fisica_em_ingles: string[]; }`;
  return result;
};

export const personagemPrompt = (prompt: string) => {
  let result = `preencha o json com os dados de um personagem descrito por "${prompt.trim()}". onde "descricao" é uma lista de adjetivos do personagem e "descricao_fisica_em_ingles" tem a lista em inglês das características físicas mais importantes. Adicione coisas como Espécie, gênero, cor dos olhos, cor e tamanho do cabelo, cor de pele. Digite ao menos 2 paragrafos de backstory.`;
...
export const relacaoPrompt = (category1: string, name1: string, category2: string, name2: string, prompt: string) => {
  let result = `escreva a relação entre um ${category1.trim()} ${name1.trim()} e um ${category2.trim()} ${name2.trim()} descrita por "${prompt.trim()}". adicione coisas como: como eles se conheceram, como se relacionam, como se sentem um em relação ao outro, etc.`;
  return result;
};
